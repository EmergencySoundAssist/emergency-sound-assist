...
`--hud --ble` 을 같이 켜면 화면과 진동이 **같은 판정**을 본다. 둘 다
`Pipeline._debounce` 로 잔향이 눌린 뒤의 결과를 소비하므로, 분류가 한두 청크 튀어도
화면이 깜빡이거나 진동이 끊기지 않는다. HUD 는 `FusedResult` 를, BLE 는 같은 값을
`notify.protocol.encode_fused` 로 4바이트에 담는다.
