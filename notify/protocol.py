...
def encode_alert(ev, info: dict) -> bytes:
    ...
    codex 하드닝 런타임(`process → (AlertEvent, info)`)용 인코더.
    HUD 런타임은 `FusedResult` 를 내므로 encode_fused 를 쓴다 — 둘 다 같은 4바이트를 만든다.
...
def encode_fused(fused) -> bytes:
    """FusedResult → 폰으로 보낼 4바이트. HUD 런타임(pipeline.runner)용 인코더.

    encode_alert 와 같은 포맷이라 폰 앱은 어느 런타임이 보냈는지 몰라도 된다.

    소리 종류는 `pipeline.runner.Pipeline._debounce` 가 잔향으로 눌러 준 뒤의 라벨이다
    (원시 분류가 아님) — 사이렌 유지 중 한두 청크 튀어도 진동이 끊기지 않는다.
    긴급이 끝나면 잔향 만료와 함께 sound=0 이 나가 폰이 진동을 멈춘다.

    SoundClass 는 str Enum('siren'/'horn')이라 _KIND_SOUND 를 그대로 쓴다.
    """
    sound = _KIND_SOUND.get(fused.sound.label, 0)
    direction = _DIR.get(fused.direction.direction, 0xFF)
    motion = _MOTION.get(fused.approach.motion, 0xFF)
    conf = max(0, min(100, int(round((fused.sound.confidence or 0.0) * 100))))
    return bytes([sound, direction, motion, conf])
