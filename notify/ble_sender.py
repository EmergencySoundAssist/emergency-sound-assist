...
from notify.protocol import (
    encode_alert, encode_fused, SERVICE_UUID, ALERT_CHAR_UUID, DEVICE_NAME,
)
...
class BleSender:
    ...
    def send_fused(self, fused) -> None:
        """전송(비차단). HUD 런타임의 FusedResult → 4바이트. send_alert 와 같은 포맷."""
        self._enqueue(encode_fused(fused))
