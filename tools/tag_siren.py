...
※ 지금은 통합 수집 모드가 권장이다: `python main.py --mic --hud --collect`
  (감지·HUD 를 그대로 돌리면서 감지 시 자동 녹음 + 화면 버튼 라벨링 + 미검출
   수동 녹음까지 — collect/collector.py). 이 도구는 HUD 를 세워야 해서 수집 중
   경고를 못 쓰는 구식 흐름이고, 화면 없는 환경의 폴백으로만 남긴다.
