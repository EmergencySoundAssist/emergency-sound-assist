...
# 캡처 주기(초). 검출 격자가 곧 경보 지연의 하한이다 — 사이렌이 청크 중간에 시작하면
# 다음 경계까지 그냥 기다린다. 실측(AI-Hub 사이렌 40건을 실차 소음 위에 얹어 재현):
#   1.00초 격자 → 지연 중앙 3.00초 · 최대 7.00초
#   0.25초 격자 → 지연 중앙 1.50초 · 최대 6.25초
...
#
# ★ 0.25초는 한 번 배포했다가 현장에서 되돌린 값이다("계속 사이렌이래"). 원인은 격자가
#   아니라 **게이트가 없던 것**이었다 — 격자를 4배로 촘촘하게 하면 초당 발화 기회도
#   4배라, 틱 오탐률이 같아도 클립당 한 번은 걸린다(오탐 클립 22→32/142 실측).
